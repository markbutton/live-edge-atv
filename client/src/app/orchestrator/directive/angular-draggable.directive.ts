import {
  Directive, ElementRef, Renderer2, Input, Output, OnInit,
  HostListener, EventEmitter, OnChanges, SimpleChanges
} from '@angular/core';
...
export class AngularDraggableDirective implements OnInit, OnChanges {
  ...
  @Output() started = new EventEmitter<any>(true);
  @Output() stopped = new EventEmitter<any>(true);
  @Output() edge = new EventEmitter<any>(true);
  ...
  @Output() movingOffset = new EventEmitter<IPosition>(true);

  /** Emit position offsets when put back */
  @Output() endOffset = new EventEmitter<IPosition>(true);
  ...
  onMouseDown(event: MouseEvent | TouchEvent, targetEl: Element) {
    // 1. skip right click;
    if (event instanceof MouseEvent && event.button === 2) {
      return;
    }
    // 2. if handle is set, the element can only be moved by handle
    if (this.handle !== undefined && !this.checkHandleTarget(targetEl, this.handle)) {
      return;
    }
