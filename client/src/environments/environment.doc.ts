export const environment = {
  production: false,
  appId: 'f9ae7ad578a6d06724a140168b4dfc95',
  baseUrl: 'http://localhost:3001/api/',
  jobAPIUrl: 'http://localhost:3005/api/',
  docsUrl: 'http://localhost:3001/api-docs/',
  busUrl: 'http://localhost:8080/',
  socketUrl: 'http://localhost:3001',
  sourceVideoUrl: 'https://develop-api.atv.spectrumtoolbox.com/api/s3',
  targetVideoUrl: 'https://develop-api.atv.spectrumtoolbox.com/api/s3',
  logsUrl: 'https://develop-log-api.atv.spectrumtoolbox.com/logs/',
  baseHref: '/',
  logSocket: 'http://localhost:3003',
  apiHeaderKey: 'x-charter-api-key',
  apiHeaderValue: 'live_edge@2019',
  polyUrl: 'http://localhost:3004/api/'
};
